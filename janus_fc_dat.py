...
#               return self.__dict__['_'+key]
	...
	def calc_curr( self, m, q, v0, n, dv, w, fn, fv, s_db, key=None ) :
		...
		b_mag = linalg.norm( self._vec_b[key] )

		if ( fn is None ) :
			nn = n
		else :		
			nn = n + fn * linalg.norm(
			                        self._vec_b[key] ) / self._avg_b
...
#			                           for i in range( len( db ) ) ]
			...
			1.e15 *sqrt( const['mu_0'] * const['m_p'] * nn )*dv/b_mag )
			                           for i in range( len( db ) ) ]
