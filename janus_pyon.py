...
#			arr=arr.lower()
		...
		for e in arr :
			...
			elif ( e.lower() in COMP2 ) :

				if ( elem['comp2'] is None ) :
					elem['comp2'] = e.lower()
				else :
					return None
...
#				return self.sig_fn
	...
	def __setitem__( self, key, value ) :
		...
		elif ( key == 'b0_fields_raw' ) :

			self.b0_fields_raw = None

			if ( value is not None ) :

				self.b0_fields_raw =  value

		elif ( key == 'b0_fields_smt' ) :

			self.b0_fields_smt = None

			if ( value is not None ) :

				self.b0_fields_smt =  value

		elif ( key == 'b0_fields_avg' ) :

			self.b0_fields_avg = None

			if ( value is not None ) :

				self.b0_fields_avg =  value

		elif ( key == 'b0_fields_db' ) :

			self.b0_fields_db = None

			if ( value is not None ) :

				self.b0_fields_db =  value
